...
func TryParseFromFile(file string) (*mapstate.MapState, []direction.Direction, error) {
	...
	if splitIdx == -1 {
		return nil, nil, fmt.Errorf("can not find empty split line from file '%s'", file)
	}

	mapState, err := mapstate.TryParseFromLines(lines[0:splitIdx])
	if err != nil {
		return nil, nil, fmt.Errorf("can not parse map from file '%s': %s", file, err.Error())
	}
	...
	for lineIdx := splitIdx + 1; lineIdx < len(lines); lineIdx++ {
		for _, r := range lines[lineIdx] {
			...
			if err != nil {
				return nil, nil, fmt.Errorf("can not parse direction from input file '%s' at line %d: %s", file, lineIdx, err.Error())
			}
